...
const ExpenseInput: React.FC<ExpenseInputProps> = ({
  ...
  const [pendingFieldIdForNewCategory, setPendingFieldIdForNewCategory] =
    useState<string | null>(null);
  const [pendingFieldIdForNewAccount, setPendingFieldIdForNewAccount] =
    useState<string | null>(null);
  ...
  useEffect(() => {
    if (
      accounts.length > 0 &&
      (!selectedAccount || !accounts.includes(selectedAccount))
    ) {
  ...
  const updateExpenseField = (
    id: string,
    field: Partial<ExpenseInputField>
  ) => {
    setExpenseFields(
    ...
    if (newCategory.trim() && !categories.includes(newCategory.trim())) {
      ...
      if (pendingFieldIdForNewCategory) {
        setExpenseFields((prev) =>
          prev.map((f) =>
            f.id === pendingFieldIdForNewCategory
              ? { ...f, category: created }
              : f
          )
        );
  ...
  const removeCategory = (categoryToRemove: string) => {
    ...
    if (
      categories.length > 1 &&
      !expenseFields.some((field) => field.category === categoryToRemove)
    ) {
  ...
  const addNewAccount = () => {
    if (newAccount.trim() && !accounts.includes(newAccount.trim())) {
      ...
      if (pendingFieldIdForNewAccount) {
        setExpenseFields((prev) =>
          prev.map((f) =>
            f.id === pendingFieldIdForNewAccount
              ? { ...f, account: created }
              : f
          )
        );
  ...
  const removeAccount = (accountToRemove: string) => {
    if (
      accounts.length > 1 &&
      !expenseFields.some((field) => field.account === accountToRemove)
    ) {
      ...
      {isExpanded && (
        ...
        >
          <div className="flex justify-between items-center text-lg font-semibold">
            <span>
              Total {expenseType === "fixed" ? "Fixed" : "Discretionary"}{" "}
              Expenses:
            </span>
            <span
              className="text-2xl"
